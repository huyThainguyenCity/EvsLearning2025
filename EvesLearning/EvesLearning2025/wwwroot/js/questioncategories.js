$(document).ready(function () {
                ...
                data.forEach(item => {
                    ...
                    const row = `
                        <tr>
                            ...
                            <td>${getValue(item.Contents)}</td>
                            <td>${getValue(item.Contents2)}</td>
    ...
    addNewForm.on("submit", function (event) {
        ...
        $.ajax({
            url: "https://localhost:7118/api/Question/QuestionCategories",
            type: "POST",
            contentType: "application/json",
            data: JSON.stringify(newQuestionLevel),
            success: function (data) {
                console.log("New Question Categories added:", data);
